...
#==========================================
# QUANTIFICATION OF SEQUENCE TURNOVER ALONG GRADIENTS
# By Melissa Chen
# Version 17-04-10
...
# def sortBinsFM(X,Y,listAbund): # Same sorting abundances, except with one boundary only
# 	lista = []
# 	listb = []
# 	listc = []
# 	bValueGrad = []
...
# def bLineEquation(X,Y,aMean,cMean,bValueGrad):
# 	m =(float(aMean) - cMean)/(float(X)-Y) # Calculate slope
# 	b = aMean-m*X # Calculate y intercept
# 	bLine = []
# 	for i in bValueGrad:
# 		bLine.append(m*i+b) # Calculate y for equation
# 	return bLine,m,b

# def makeAModelFM(X,Y,binValues,bValueGrad):
# 	aMean = average(binValues['lista'])
# 	cMean = average(binValues['listc'])
# 	bLine,m,b = bLineEquation(X,Y,aMean,cMean,bValueGrad)
# 	# Make a combined 'piecewise' list that has each value and its corresponding mean
# 	abundance = binValues['lista'] + binValues['listb'] + binValues['listc']
...
# def scaleBySig(X,Y,pAB,pBC): # TBD: Should fix this
...
# 	scaledBoundary = round((X*aScale + Y*bScale),1)
# 	scaledBoundary = average([X,Y])
# 	return scaledBoundary # Output is the scaled Boundary based on significance
	
def scaleByDiff(X,Y,meanA,meanB,meanC):
	if meanA > meanC:
		diffX = meanA-meanB
		diffY = meanB-meanC
		diffXY = Y-X
...
# 				pass
# 			else:
# 				combined = makeAModelFM(X,Y,binValues,bValueGrad)
...
# 						bestfitAB.append([X,Y]) # Assumes they are due to some lack of data, not that they are exactly the same
...
def typeTaxa(X, Y, listAbund): # Uses Welch's t-test and bins above to classify taxa as marine, fresh, or brackish (or other)
	global threshold
	global Low,Inter,High
...
# 		print "NO VARIANCE IN ONE OF GROUPS"
	...
	if average(groupA) == average(groupC) and numpy.var(groupA) == 0 and numpy.var(groupC) == 0:
		...
		pAC = stats.ttest_ind(groupA,groupC, equal_var = False)[1] # p-value of X vs C
	if average(groupA) == average(groupB) and numpy.var(groupA) == 0 and numpy.var(groupB) == 0:
		pAB = 1
	else:
		pAB = stats.ttest_ind(groupA,groupB, equal_var = False)[1] # p-value of X vs Y
	if average(groupB) == average(groupC) and numpy.var(groupB) == 0 and numpy.var(groupC) == 0:
		pBC = 1
	else:
		pBC = stats.ttest_ind(groupB,groupC, equal_var = False)[1] # p-value of Y vs C
# 	print pAC,pAB,pBC
...
# 	typeOutput['sigAC'] = pAC # Save in output file
	...
	typeOutput['X'] = X
	typeOutput['Y'] = Y
	isBrack = False	# See if there is a brackish community or not; if there isn't, then I compare just X and C.
	if sigAB and sigBC: # When the middle group is REAL (and not just 1 number), and it is significantly different than both flanking groups
		if meanB > meanA and meanB > meanC: # brackish water species
			...
			typeOutput['Y'] = Y
 			...
 			pass	# All other combos mean the brackish do NOT exist, so we can just compare X and C
...
# 		typeOutput['type'] = 'noclass' # X 'catch-all' for things that have NO significant differences between any of the groups
...
# 	 			typeOutput['bloom'] = 'No'
		...
		# Above, you can have EITHER X>C or Y>C but they must be significant, and if it's Y, the distance between Y and C must be greater than the distance between X and Y (to prevent brackish-looking ones)
...
def validateTypeTest(typeSimple, meanA, meanB, meanC): # Test to see if the data agrees with classification
	...
	if typeSimple == 'noclass':
		...
		elif typeSimple == Inter:
			isValidate = ((meanB > meanC) and (meanB > meanA))
		elif typeSimple == High:
...
parser.add_argument(
	'--minA',
	help = "Minimum boundary X can be [default is minVal]",
	required = False,
	default = 'Check')
parser.add_argument(
	'--maxB',
	help = 'Maximum boundary Y can be [default is maxVal]',
	required = False,
	default = 'Check')
parser.add_argument(
	'--ABdiff',
...
parser.add_argument(
	...
	'--check_class',
	action =  'store_true',
	help = 'Include this flag if you want the script to double-check classifications at end. [Default: False]',
	required = False,
	default = False)
parser.add_argument(
	'-v',
