...
# # For testing
# # 
# boundariesPWD <- "boundaries.txt"
# modelboundariesPWD <- "modelBoundaries_type.txt"
# taxaAbundPWD <- "taxa_abundances_across_gradient.txt"
# typesPWD <- "types_across_gradient"
# gradientNamesPWD <- "gradient.txt"
# LOGPWD <- "LOG.txt"
# allBins <- TRUE
# condensedBins <- TRUE
# threshold <- 0.10
...
jpeg(file = "BoundaryDistributions.jpg")
hist(histBoundaries
...
for (x in (1:nrow(taxaAbundances))) {
  ...
  if (typeMB != 'noclass') {
    typeMB = typeMB
  } else if (typeMB == 'noclass' & modelBoundaries[taxa,'bloom'] != 'No') {
    typeMB <-paste0("bloom",modelBoundaries[taxa,'bloom'])
  } else {
    typeMB <- "noclass"
  }
  bloom <- modelBoundaries[taxa, 'bloom']
  jpeg(file = paste0(typeMB,'_Indiv',taxa,'.jpeg'))
...
colorsGradient <- c("blue","darkblue","lightblue","cyan","purple","green","magenta","pink","orange","red","darkred","black", "grey")
colorsGradientCondensed <- c("blue","purple","red","black")


if (allBins == TRUE) {
  typesAll <- as.matrix(typesAll)
  jpeg(file = "taxatypesacrossgradient_all.jpeg")
  ...
  barplot(typesAll, beside = FALSE
          ...
          , xlab = 'Gradient'
          , ylab = 'Relative Abundance')
  legend('right', inset = c(-0.3,0), legend = rev(rownames(typesAll)), pch = 19, col = rev(colorsGradient))
  dev.off()
}

if (condensedBins == TRUE) {
  typesCondensed <- as.matrix(typesCondensed)
  jpeg(file = "taxatypesacrossgradient_condensed.jpeg")
  par(mar=c(5.1,4.1,4.1,8.1), xpd = TRUE)
  barplot(typesCondensed, beside = FALSE
          ...
          , xlab = 'Gradient'
          , ylab = 'Relative Abundance')
  legend('right', inset = c(-0.3,0), legend = rev(rownames(typesCondensed)), pch = 19, col = rev(colorsGradientCondensed))
  dev.off()
}
...
if (allBins == TRUE) {
  for (i in 1:nrow(typesAll)){
    jpeg(file = paste0('./TaxaAbund_bytype_all/Indiv_', rownames(typesAll)[i], '_all.jpeg'))
...
if (condensedBins == TRUE) {
  for (i in 1:nrow(typesCondensed)){
    jpeg(file = paste0('./TaxaAbund_bytype_condensed/Indiv_', rownames(typesCondensed)[i], '.jpeg'))
...
jpeg(file = paste0('./TaxaAbund_bytype_condensed/Indiv_', gradientNames[1],gradientNames[3], '.jpeg'))
...
lowOnlyTrue <- modelBoundaries[grep(paste0("^",gradientNames[1],"$"), modelBoundaries$type),]
lowOnlyHalf <- modelBoundaries[grep(paste0("half",gradientNames[1]), modelBoundaries$type),]
lowOnlyBloom <- modelBoundaries[grep(paste0(gradientNames[1]), modelBoundaries$bloom),]


highOnly <- modelBoundaries[grep(paste0(gradientNames[3]), modelBoundaries$typeSimple),]
highOnlyTrue <- modelBoundaries[grep(paste0("^",gradientNames[3],"$"), modelBoundaries$type),]
highOnlyHalf <- modelBoundaries[grep(paste0("half",gradientNames[3]), modelBoundaries$type),]
highOnlyBloom <- modelBoundaries[grep(paste0(gradientNames[3]), modelBoundaries$bloom),]

interOnly <- modelBoundaries[grep(paste0(gradientNames[2]), modelBoundaries$typeSimple),]
interOnlyTrue <- modelBoundaries[grep(paste0("^",gradientNames[2],"$"), modelBoundaries$type),]
interOnlyHalf <- modelBoundaries[grep(paste0("half",gradientNames[2]), modelBoundaries$type),]
interOnlyBloom <- modelBoundaries[grep(paste0(gradientNames[2]), modelBoundaries$bloom),]
interOnlyHi <- modelBoundaries[grep(paste0("hi",gradientNames[2]), modelBoundaries$type),]
interOnlylo <- modelBoundaries[grep(paste0("lo",gradientNames[2]), modelBoundaries$type),]
...
if (condensedBins == TRUE) {
  ...
  if (length(allLow) > 0){
    ...
    jpeg(file = paste0("./Histograms/Histogram_",gradientNames[1],"Boundaries_all.jpeg"))
  ...
  if (length(nonBloomLow) > 0){
    ...
    meanLow <- round(mean(lowBoundaries),2)
    jpeg(file = paste0("./Histograms/Histogram_",gradientNames[1],"Boundaries_nonBloom.jpeg"))
    hist(lowBoundaries
         , xlim = range(minGrad,maxGrad)
         , xlab = "Gradient"
         , col = "grey"
...
if (allBins == TRUE) {
  ...
  if (length(TrueLow) > 0){
    ...
    meanLow <- round(mean(lowBoundaries),2)
    jpeg(file = paste0("./Histograms/Histogram_",gradientNames[1],"Boundaries_True.jpeg"))
    hist(lowBoundaries
         , xlim = range(minGrad,maxGrad)
         , xlab = "Gradient"
         , breaks = binBreaks
...
print("REACH HERE")
  ...
  if (length(HalfLow) > 0){
    ...
    meanLow <- round(mean(lowBoundaries),2)
    jpeg(file = paste0("./Histograms/Histogram_",gradientNames[1],"Boundaries_Half.jpeg"))
    hist(as.numeric(lowBoundaries)
         , xlim = range(minGrad,maxGrad)
         , xlab = "Gradient"
         , col = "grey"
  ...
  if (length(BloomLow) > 0){
    ...
    meanLow <- round(mean(lowBoundaries),2)
    jpeg(file = paste0("./Histograms/Histogram_",gradientNames[1],"Boundaries_Bloom.jpeg"))
    hist(lowBoundaries
         , xlim = range(minGrad,maxGrad)
         , xlab = "Gradient"
         , breaks = binBreaks
...
if (condensedBins == TRUE) {
  ...
  if (length(allHigh) > 0){
    ...
    jpeg(file = paste0("./Histograms/Histogram_",gradientNames[3],"Boundaries_all.jpeg"))
    hist(HighBoundaries
         , xlim = range(minGrad,maxGrad)
         , xlab = "Gradient"
         , breaks = binBreaks
  ...
  if (length(nonBloomHigh) > 0){
    ...
    meanHigh <- round(mean(HighBoundaries),2)
    jpeg(file = paste0("./Histograms/Histogram_",gradientNames[3],"Boundaries_nonBloom.jpeg"))
    hist(HighBoundaries
         , xlim = range(minGrad,maxGrad)
         , xlab = "Gradient"
         , breaks = binBreaks
...
if (allBins == TRUE) {
  ...
  if (length(TrueHigh) > 0){
    ...
    meanHigh <- round(mean(HighBoundaries),2)
    jpeg(file = paste0("./Histograms/Histogram_",gradientNames[3],"Boundaries_True.jpeg"))
    hist(HighBoundaries
         , xlim = range(minGrad,maxGrad)
         , xlab = "Gradient"
         , breaks = binBreaks
  ...
  if (length(HalfHigh) > 0) {
    ...
    meanHigh <- round(mean(HighBoundaries),2)
    jpeg(file = paste0("./Histograms/Histogram_",gradientNames[3],"Boundaries_Half.jpeg"))
    hist(HighBoundaries
         , xlim = range(minGrad,maxGrad)
         , xlab = "Gradient"
         , breaks = binBreaks
  ...
  if (length(BloomHigh) > 0) {
    ...
    meanHigh <- round(mean(HighBoundaries),2)
    jpeg(file = paste0("./Histograms/Histogram_",gradientNames[3],"Boundaries_Bloom.jpeg"))
    hist(HighBoundaries
         , xlim = range(minGrad,maxGrad)
         , xlab = "Gradient"
         , breaks = binBreaks
...
if (condensedBins == TRUE) {
  ...
  if (length(allInter) > 0){
    ...
    jpeg(file = paste0("./Histograms/Histogram_",gradientNames[2],"Boundaries_all.jpeg"))
  ...
  if (length(nonBloomInter) > 0){
    ...
    meanInterTwo <- round(mean(InterBoundaries[,2]),2)
    maximumY <- max(max(table(InterBoundaries[,1])),max(table(InterBoundaries[,2])))*1.5
    jpeg(file = paste0("./Histograms/Histogram_",gradientNames[2],"Boundaries_nonBloom.jpeg"))
    hist(InterBoundaries[,1]
         , xlim = range(minGrad,maxGrad)
         , xlab = "Gradient"
         , ylim = c(0,maximumY)
...
if (allBins == TRUE) {
  ...
  if (length(TrueInter) > 0){
    ...
    meanInterTwo <- round(mean(InterBoundaries[,2]),2)
    maximumY <- max(max(table(InterBoundaries[,1])),max(table(InterBoundaries[,2])))*1.5
    jpeg(file = paste0("./Histograms/Histogram_",gradientNames[2],"Boundaries_True.jpeg"))
    hist(InterBoundaries[,1]
         , xlim = range(minGrad,maxGrad)
         , xlab = "Gradient"
         , ylim = c(0,maximumY)
  ...
  if (length(HalfInter) > 0){
    ...
    meanInterTwo <- round(mean(InterBoundaries[,2]),2)
    maximumY <- max(max(table(InterBoundaries[,1])),max(table(InterBoundaries[,2])))*1.5
    jpeg(file = paste0("./Histograms/Histogram_",gradientNames[2],"Boundaries_Half.jpeg"))
    hist(InterBoundaries[,1]
         , xlim = range(minGrad,maxGrad)
         , xlab = "Gradient"
         , ylim = c(0,maximumY)
  ...
  if (length(BloomInter) > 0){
    ...
    meanInterTwo <- round(mean(InterBoundaries[,2]),2)
    maximumY <- max(max(table(InterBoundaries[,1])),max(table(InterBoundaries[,2])))*1.5
    jpeg(file = paste0("./Histograms/Histogram_",gradientNames[2],"Boundaries_Bloom.jpeg"))
    hist(InterBoundaries[,1]
         , xlim = range(minGrad,maxGrad)
         , xlab = "Gradient"
         , ylim = c(0,maximumY)
  ...
  if (length(HiInter) > 0){
    ...
    meanInterTwo <- round(mean(InterBoundaries[,2]),2)
    maximumY <- max(max(table(InterBoundaries[,1])),max(table(InterBoundaries[,2])))*1.5
    jpeg(file = paste0("./Histograms/Histogram_",gradientNames[2],"Boundaries_Hi.jpeg"))
    hist(InterBoundaries[,1]
         , xlim = range(minGrad,maxGrad)
         , xlab = "Gradient"
         , ylim = c(0,maximumY)
  ...
  if (length(LoInter) > 0){
    ...
    meanInterTwo <- round(mean(InterBoundaries[,2]),2)
    maximumY <- max(max(table(InterBoundaries[,1])),max(table(InterBoundaries[,2])))*1.5
    jpeg(file = paste0("./Histograms/Histogram_",gradientNames[2],"Boundaries_Lo.jpeg"))
    hist(InterBoundaries[,1]
         , xlim = range(minGrad,maxGrad)
         , xlab = "Gradient"
         , ylim = c(0,maximumY)
...
system("mkdir IndividualPlots")
system('ls | grep -e "[a-z]_Indiv*"| while read f; do mv $f ./IndividualPlots; done')
...
# ?replace
# # Plot only the 'major taxa'
# grthanTF <- gsub("NA","FALSE",as.character(grthan))
# grthanTF <- replace(grthan, is.na(grthan)[1549], FALSE)
# grthanTF[1549]
taxaAbund.grad.filt <- taxaAbund.grad[grthanTF,]
# 1549
# grthanTF[1549]
# grep('25148', rownames(taxaAbund.grad.filt))
# grep('^25148$', rownames(taxaAbund.grad))
# nrow(taxaAbund.grad)
# max(taxaAbund.grad.filt[70,])
# rownames(taxaAbund.grad)[1520:1530]
# grthanTF[70]
...
for (i in (1:nrow(taxaAbund.grad.filt))) {
  jpeg(paste0('./',output,'/IndividualPlots/Indiv-',row.names(taxaAbund.grad.filt)[i],typesforIndiv[i],'.jpg'))
  ...
  # assign(sub("^.*__.*__","", paste0(row.names(taxaAbund.grad.filt)[i],".lo")), loess(as.numeric(taxaAbund.grad.filt[i,]) ~ ordered.gradient))
  ...
  # # Store the polynomial names and fits in a file so I can check them later if I want to
  # fits[i,1] <- sub("^.*__.*__","", paste0(row.names(taxaAbund.grad)[i],".lo"))
  # fits[i,2] <- summary(get(sub("^.*__.*__","", paste0(row.names(taxaAbund.grad)[i],".lo"))))$s
  dev.off()
}
# names(fits) <- c("Taxa","ResStError")
...
jpeg(paste0('./',output,'/Taxadistributions.jpg'), width = 1000, height = 500)
...
for (i in (1:nrow(taxaAbund.grad.filt))) {# First, make function that gets rid of 'outliers'
  ...
  # Store the polynomial names and fits in a file so I can check them later if I want to
  # fits[i,1] <- sub("^.*__.*__","", paste0(row.names(taxaAbund.grad.filt)[i],".lo"))
  # fits[i,2] <- summary(get(sub("^.*__.*__","", paste0(row.names(taxaAbund.grad.filt)[i],".lo"))))$s
}
dev.off()
...
# typeUnique <- typeUnique[-grep("^noclass$",typeUnique)]
...
for (i in 1:length(typeUnique)){
  ...
  # assign(paste0(typeUnique[i],'abundances'), tempAbundances)
  # taxaAbundTypes <- rbind(taxaAbundTypes,get(paste0(typeUnique[i],'abundances')))
...
colorsApproved <- c("blue","yellow","cyan","purple","lightgreen","magenta","lightblue","pink","orange","red", "green","black","gray")
...
# First, make an empty plot with the limits set as the maximum found in my data.frame
jpeg(paste0('./',output,'/TaxadistributionsRelative.jpg'), width = 750, height = 500)
# par(mar = c(4,4,1,1), oma = c(2,2,2,2))
...
# First, make an empty plot with the limits set as the maximum found in my data.frame
jpeg(paste0('./',output,'/TaxadistributionsRelative_condensed.jpg'), width = 750, height = 500)
# par(mar = c(4,4,1,1), oma = c(2,2,2,2))
par(fig=c(0,0.67,0,1))
plot(x = NULL, y = NULL
     , xlim = c(1,max(ordered.gradient, na.rm = TRUE))
...
# First, make an empty plot with the limits set as the maximum found in my data.frame
jpeg(paste0('./',output,'/TaxadistributionsRelativeAggregate.jpg'), width = 750, height = 500)
# par(mar = c(4,4,1,1), oma = c(2,2,2,2))
par(fig=c(0,0.67,0,1))
plot(x = NULL, y = NULL
     , xlim = c(1,max(ordered.gradient, na.rm = TRUE))
...
jpeg(paste0("./",output,"/TaxaSummariesPartitioned.jpg"), width = 1000, height = 500)
...
for (i in 1:nrow(editedModelBoundaries)) {
  if (editedModelBoundaries$typeSimple[i] == as.character(gradientNames[1])) {
    editedModelBoundaries$START[i] <- minGrad
    editedModelBoundaries$STOP[i] <- editedModelBoundaries$boundaries[i]
  } else if (editedModelBoundaries$typeSimple[i] == as.character(gradientNames[3])) {
    editedModelBoundaries$START[i] <- editedModelBoundaries$boundaries[i]
    editedModelBoundaries$STOP[i] <- maxGrad
  } else if (editedModelBoundaries$typeSimple[i] == as.character(gradientNames[2])) {
    editedModelBoundaries$START[i] <- editedModelBoundaries$boundaries[i]
...
jpeg(file = "OTUrangelimits_Ordered_all.jpeg", width = 500, height = 1000)
plot(NULL
     , xlim = c(minGrad,maxGrad)
     , ylim = c(0,100)
     , xlab = "Salinity"
...
jpeg(file = "OTUrangelimits_OrderedCondensed.jpeg", width = 500, height = 1000)
plot(NULL
     , xlim = c(minGrad,maxGrad)
     , ylim = c(0,100)
     , xlab = "Salinity"
...
# Get subset only
loLinesSimp <- editedModelBoundaries[grep(paste0("^",gradientNames[1],"$"), editedModelBoundaries$type),]
interLinesSimp <- editedModelBoundaries[grep(paste0("^",gradientNames[2],"$"), editedModelBoundaries$type),]
hiLinesSimp <- editedModelBoundaries[grep(paste0("^",gradientNames[3],"$"), editedModelBoundaries$type),]
...
jpeg(file = "OTUrangelimits_Ordered_PureOnly.jpeg", width = 500, height = 1000)
plot(NULL
     , xlim = c(minGrad,maxGrad)
     , ylim = c(0,100)
     , xlab = "Salinity"
